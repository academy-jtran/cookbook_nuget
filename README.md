...
For users of Chef 10 without the `nuget_script` built-in resource, this cookbook also includes a resource/provider for executing scripts using the PowerShell interpreter.
...
**Note**: In Chef 11, use the built-in [nuget_script](http://docs.opscode.com/resource_nuget_script.html) resource.
...
nuget "write-to-interpolated-path" do
  code <<-EOH
  $stream = [System.IO.StreamWriter] "#{Chef::Config[:file_cache_path]}/nuget-test.txt"
...
nuget "cwd-then-write" do
  cwd Chef::Config[:file_cache_path]
  code <<-EOH
  $stream = [System.IO.StreamWriter] "C:/nuget-test2.txt"
  $pwd = pwd
...
Include the `nuget2` recipe in a run list, to ensure PowerShell 2.0 is installed.
...
Include the `nuget3` recipe in a run list, to install PowerShell 3.0 is installed on applicable platforms. If a platform is not supported or if it already includes PowerShell 3.0, an exception will be raised.

### nuget4

Include the `nuget4` recipe in a run list, to install PowerShell 4.0 is installed on applicable platforms. If a platform is not supported or if it already includes PowerShell 4.0, an exception will be raised.

### nuget5

Note: Windows Management Framework 5 is in preview.

Include the `nuget5` recipe in a run list, to install PowerShell 5.0 is installed on applicable platforms. If a platform is not supported or if it already includes PowerShell 5.0, an exception will be raised.
